...
function compile(module_code, space, open_tag, close_tag) {
    ...
    if (space === 'preserve') {
        arr = module_code.replace(/[\r\t]/g, '').split(o).join(c+'\x00').split(c);
        for (m = 0, l = arr.length; m < l; m++) {
            if (arr[m].charAt(0) !== '\x00') {
                str += (pa ? "+'" : ";$+='") + arr[m].replace(/(\\|')/g, '\\$1').split('\n').join('\\r\\n') + "'";
                pa = true;
            } else if (arr[m].charAt(1) === '!') {
                str += (pa ? "+(" : ";$+=(") + arr[m].substring(2).trim().replace(/\n/g, '') + ')';
                pa = true;
            } else if (arr[m].charAt(1) === '=') {
                str += (pa ? "+(" : ";$+=_e(") + arr[m].substring(2).trim().replace(/\n/g, '') + ')';
                pa = true;
            } else {
        ...
        arr = module_code.replace(/[\r\t\n]/g, '').replace(/ +/g, ' ').split(o).join(c+'\x00').split(c);
        for (m = 0, l = arr.length; m < l; m++) {
            if (arr[m].charAt(0) !== '\x00') {
                str += (pa ? "+'" : ";$+='") + arr[m].replace(/(\\|')/g, '\\$1').trim() + "'";
                pa = true;
            } else if (arr[m].charAt(1) === '!') {
                str += (pa ? "+(" : ";$+=(") + arr[m].substring(2).trim() + ')';
                pa = true;
            } else if (arr[m].charAt(1) === '=') {
                str += (pa ? "+(" : ";$+=_e(") + arr[m].substring(2).trim() + ')';
                pa = true;
            } else {
